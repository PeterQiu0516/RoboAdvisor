...
def getLoginRequest():
    ...
    q9 = request.values.getlist("q9")
    q10 = request.values.getlist("q10")
    if q1!=None and q2!=None and q3!=None and q4!=None and q5!=None and q6!=None and q7!=None and q8!=[] and q9!=[]and q10!=[]:
        ...
        import pandas
        # data=pandas.read_csv("2.csv",header=None).astype(str)
        # # stats = importr('stats')
        # # base = importr('base')
        # b=robjects.FloatVector(a)
...
#         robjects.r('''
#                    f <- function(x,y){1 * x + 2 * y}
#                    f <- function(data,ins){data[2,2]}
# ''')


        #from rpy2.robjects.packages import importr
        #base = importr("base")
        #temp=base.as_Date(data.iloc[:,0],'%Y/%m/%d')
...
weights<-function(data) {
    ...
    write.csv(data_print_out_final,"output.csv",na="")
    return (w)
}
...
re=R_final(data,10)
f<-function(){return(re)}

#data<-read.csv("test.csv",header=TRUE,sep=",")
#print(R_final(data,2))

        ''')

        # outfund_all=robjects.r['first_filter'](data,5)
        # final_fund_all=robjects.r['second_filter'](outfund_all,5)
        # c=robjects.r['weights'](final_fund_all)
        c = robjects.r['f']()


        return render_template('hello.html',result=c)
    else:
        return render_template('option.html')
