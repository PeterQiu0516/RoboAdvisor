...
filter<-function(data,sour,ac,gf,riskave){
  ...
  for(i in 1:4){
    if(i>3){next}
    acfilter<-switch(ac[i],match(data[which(data$Asset.Class == "Equity"), ][,1],data[,1]),
  ...
  for(i in 1:7){
    if(i>4 & cn>50){break}
    gffilter<-switch(gf[i],match(data[which(data$Geographical.Focus == "Africa& Middle West Region"), ][,1],data[,1]),
...
R_final(c(1,2,3,4),c(2,4,3,1,6,5,7),3)
