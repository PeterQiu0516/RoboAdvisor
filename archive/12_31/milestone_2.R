...
sorted_risk_all<-sort(risk_all,decreasing=FALSE) #in decreasing order  3£º2*79+1:3*79 
...
for(k in selected_index){
  ...
  write.csv(outfund_all,"fund_3.csv",na="")
return (outfund_all)}

outfund_all<-first_filter(data,1)
...
second_filter<-function(data,ins){
data2<-data.frame()
return_all<-c()
fund_num<-ncol(data)
...
selected_index<-match(head(sorted_return_all,ins),return_all)

for(k in selected_index){
  final_fund<-data2[,(fund_num+1-k)]
...
write.csv(final_fund_all,"fund_4.csv",na="")
